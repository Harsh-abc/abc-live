										...
										<div class="card-body p-4">
											<div class="row align-items-center">
												<div class="offset-xl-4 offset-sm-6 col-xl-8 col-sm-6 col-12 img-bg ">
													...
													<p class="tx-white-7 mb-1">You have <b class="text-warning">{{$TodayEnquiries}}</b> New Inquiries. </p>
									...
									<div class="card custom-card">
										<div class="card-body">
											<div class="card-item">												
												...
												<div class="card-item-body">
													<div class="card-item-stat">
														<h4 class="font-weight-bold">{{$careersCount}} </h4> <small><b class="text-success"> </b> Include All</small> </div>
												</div>
											</div>
										</div>
									</div>
									...
									<div class="card custom-card">
										<div class="card-body">
											<div class="card-item">
												...
												<div class="card-item-body">
													<div class="card-item-stat">
														<h4 class="font-weight-bold">{{ $Todaycareers }}</h4> <small><b class="text-danger"> </b> Include All</small> </div>
												</div>
											</div>
										</div>
									</div>
											...
											<div class="table-responsive tasks">
												<table class="table card-table table-vcenter mb-0  border">
													...
													<tbody>
														...
														<tr>
															<td class=" "> {{$item->name}} 
																<p class="mb-0 tx-11 text-muted" style="line-height:normal;"> {{ ($item->created_at) ? date('d M Y',strtotime($item->created_at)) : '' }} <small class="text-muted d-block">{{ ($item->created_at) ? date('H:s:i',strtotime($item->created_at)) : '' }}</small> </p> 
															 </td>
															<td class=""> {{$item->contact}} </td>
															<td class=" "> <a href="mailto:<?=$item->email?>"> {{$item->email}} </a> </td>
															<td><p class="mb-0 max-width">{{$item->message}}</p></td>
														</tr>
														@endforeach
										...
										<tr>
											...
											<td>
												<div class="d-inline-block">
													<h6 class="mb-2 tx-15 font-weight-semibold"> {{$item->name}}  </h6>
													<p class="mb-0 tx-11 text-muted" style="line-height:normal;"> {{ ($item->created_at) ? date('d M Y',strtotime($item->created_at)) : '' }} <small class="text-muted d-block">{{ ($item->created_at) ? date('H:s:i',strtotime($item->created_at)) : '' }}</small> </p>
												</div>
											</td>
											<td class="text-right"> {{$item->job_role}}  <hr /> <a href="/img/career/{{$item->attachment}}" download> Resume </a> </td>
										</tr>
										@endforeach
									</tbody>
								</table>
